    ...
    return (
        <AriaCalendarCell
            ...
            className={({ isDisabled, isFocusVisible, isSelectionStart, isSelectionEnd, isSelected, isOutsideMonth }) => {
                ...
                return cx(
                    "relative size-6 focus:outline-none",
                    isRoundedLeft && "rounded-l-full",
                    isRoundedRight && "rounded-r-full",
                    isInRange && !isDisabled && "bg-red-50",
                    isSelected && isRangeCalendar && !isDisabled && "bg-red-50",
                    ...
                    isBeforeEnd &&
                    isRangeCalendar &&
                    "after:absolute after:inset-0 after:translate-x-full after:bg-gradient-to-l after:from-transparent after:to-red-50 in-[[role=gridcell]:last-child]:after:hidden",
                    ...
                    isAfterStart &&
                    isRangeCalendar &&
                    "after:absolute after:inset-0 after:-translate-x-full after:bg-gradient-to-r after:from-transparent after:to-red-50 in-[[role=gridcell]:first-child]:after:hidden",
                );
            }}
        >
            {({ isDisabled, isFocusVisible, isSelectionStart, isSelectionEnd, isSelected, formattedDate }) => {
                ...
                return (
                    <div
                        className={cx(
                            "relative flex size-full items-center justify-center rounded-full text-xs",
                            // Disabled state.
                            isDisabled ? "text-disabled" : "text-secondary hover:text-secondary_hover",
                            // Focus ring, visible while the cell has keyboard focus.
                            isFocusVisible ? "outline-2 outline-offset-2 outline-focus-ring" : "",
                            // Hover state for cells in the middle of the range.
                            isSelected && !isDisabled && isRangeCalendar ? "font-medium text-red-700" : "",
                            markedAsSelected && "bg-red-600 font-medium text-white hover:bg-red-700 hover:text-white",
                            // Hover state for non-selected cells.
                            !isSelected && !isDisabled ? "hover:bg-red-50 hover:font-medium!" : "",
